...
void loop() {
  ...
  if (millis() - 15000 > timeHolder)
  {
    ...
    if (chartValue <= 20) {  // 0-20 is red "dry"

      analogWrite(redPin, 1000);
      analogWrite(greenPin, 0);
      analogWrite(bluePin, 0);

    } else if (chartValue > 20 && chartValue <= 70) // 21-70 is green
    {
    ...
    else if (chartValue > 70 ) // 71-100 is blue
    {
  ...
  client.println("      redFrom: 0, redTo: 20,");
  client.println("      yellowFrom: 20, yellowTo: 70,");
  client.println("      greenFrom: 70, greenTo: 100,");
